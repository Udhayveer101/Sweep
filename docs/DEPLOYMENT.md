...
substitute, because it cannot sign for distribution and notarization will reject it.
...
is governed by TCC, which is not an entitlement, and the sandbox file entitlements apply only to
...
the artifact name follows automatically, as in `Sweep-1.1.0.dmg`. The Git tag is `v` plus the same
...
releases, with the quarantine attribute applied, then `spctl -a -t exec`:
...
The queue is the whole cost, and it varies, so budget for it rather than assuming the first
timing repeats.
